...
def search(item):
    ...
    # building a query
    bases = [f"{x[1]}search_{item}?" for x in CORPORA if x[0] in langs_corp]
    query = request.url.split(f"search_{item}?", maxsplit=1)[1]
...
def pagination_tables(item, page):
    """
    another function for stealing
    """
    lang, page = page.split("_")
...
def pagination_sent(page):
    return pagination("sent", page)


@app.route("/search_word/<page>")
def pagination_word(page):
    return pagination_tables("word", page)


@app.route("/search_lemma/<page>")
def pagination_lemma(page):
    return pagination_tables("lemma", page)
